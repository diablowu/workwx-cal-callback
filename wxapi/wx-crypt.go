...
type xmlProtocalProcessor struct {
}

func (p *xmlProtocalProcessor) parse(srcData []byte) (*WXBizMsg4Recv, *CryptError) {
	var ret = new(WXBizMsg4Recv)
	err := xml.Unmarshal(srcData, ret)
	if err != nil {
		return nil, NewCryptError(ParseJsonError, err.Error())
	} else {
		return ret, nil
	}
}
func (p *xmlProtocalProcessor) serialize(msg_send *WXBizJsonMsg4Send) ([]byte, *CryptError) {
	return nil, nil
}
...
func (self *JsonProcessor) parse(src_data []byte) (*WXBizMsg4Recv, *CryptError) {
...
func NewWXBizMsgCrypt(token, encoding_aeskey, receiver_id string, protocol_type ProtocolType) *WXBizMsgCrypt {
	...
	if protocol_type != JsonType {
		protocol_processor = new(xmlProtocalProcessor)
	} else {
